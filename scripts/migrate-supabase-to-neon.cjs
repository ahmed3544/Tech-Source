...
const TARGET_URL = process.env.DATABASE_URL || process.env.SUPABASE_DB_URL;
...
async function ensureTargetSchema(target) {
  ...
  console.log('Production database schema verified/created.');
}

async function main() {
  if (!TARGET_URL) {
    console.log('Database schema setup skipped: DATABASE_URL/SUPABASE_DB_URL is missing.');
    return;
  }
  ...
  try {
    // IMPORTANT: the API reads these tables directly. The old migration only
    // copied rows when the target tables already existed, so a fresh/partially
    // provisioned production database could make /api/data and notifications
    // return HTTP 500. Always create missing tables first; this is additive only.
    await ensureTargetSchema(target);

    // Nothing else to migrate when there is no separate Supabase source.
    if (!SOURCE_URL || SOURCE_URL === TARGET_URL) {
      if (!SOURCE_URL) console.log('Supabase→Neon row migration skipped: SUPABASE_DB_URL is missing.');
      else console.log('Supabase→Neon row migration skipped: source and target are identical.');
      return;
    }
          ...
          const tuples = batch.map((row, rowIndex) => {
            const placeholders = columns.map((column, columnIndex) => {
              ...
              if (targetType === 'json' || targetType === 'jsonb') {
                if (sourceType === 'json' || sourceType === 'jsonb') value = normalizeJsonText(value);
                else value = normalizeJsonText(value);
              }
