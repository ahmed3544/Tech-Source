...
if (!code.includes(marker)) {
  ...
  // The insertion is placed before pushSync declares `now`; use the current
  // clock directly so generated client code cannot hit a temporal dead zone.
  code = code.replace('const mutationTimestamp = new Date(now).toISOString();', 'const mutationTimestamp = new Date().toISOString();');
}
