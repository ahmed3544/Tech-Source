...
// Employee Portal: use the shared dictionary for department, approval status and units.
patchFile('src/components/EmployeePortal.tsx', (code) => {
  ...
  // Existing duration templates that used the Arabic short hour unit.
...
// Kiosk: keep the End Break label clean in both language branches.
...
// Attendance table: replace any remaining hard-coded Arabic short units.
...
console.log('UI localization patch applied.');
