...
// Ensure the timeline helpers are present independently. Older versions of
// this patch only checked for timeToMinutes, but the source can already have
// timeToMinutes while still missing timelinePercent.
if (!code.includes('const timelinePercent =')) {
  const marker = 'const timePercent = (value: string) => (timeToMinutes(value) / (24 * 60)) * 100;';
  const helpers = `${marker}\nconst timelinePercent = (minutes: number) => Math.max(0, Math.min(100, (Number(minutes) / 1440) * 100));\nconst breakWindows = (shift: any, start: number, end: number) => {\n  const explicit = Array.isArray(shift?.breaks) ? shift.breaks : [];\n  const windows = explicit.map((b: any) => {\n    const bs = timeToMinutes(b?.startTime ?? b?.start ?? b?.from);\n    const be = timeToMinutes(b?.endTime ?? b?.end ?? b?.to);\n    return be > bs ? { start: bs, end: be } : null;\n  }).filter(Boolean);\n  if (windows.length) return windows;\n  const minutes = Number(shift?.breakMinutes || 0);\n  if (minutes <= 0 || end <= start) return [];\n  const midpoint = start + (end - start) / 2;\n  return [{ start: Math.max(start, midpoint - minutes / 2), end: Math.min(end, midpoint + minutes / 2) }];\n};`;
  if (code.includes(marker)) code = code.replace(marker, helpers);
}
