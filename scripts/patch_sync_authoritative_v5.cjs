...
if (!code.includes(marker)) {
  // Never invent a fresh server timestamp for a mutation that did not carry its own timestamp.
  code = code.replace(
  ...
  // Attendance: an old device must not be able to recreate or overwrite a record merely because the request arrived later.
  code = code.replace(
...
async function upsertLeave`
  ...
  // Leave: same rule. Existing DB rows remain untouched when a stale client sends an unstamped snapshot.
  code = code.replace(
...
async function upsertOvertime`
  ...
  // Prevent an unstamped employee/shift/notification snapshot from being treated as newer too.
