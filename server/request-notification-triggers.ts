...
import { ensureNotificationStorage, saveOne } from './notification-system-v2.js';
...
async function insertNotification(recipientId:string, type:string, title:string, message:string, relatedEmployeeId:string, relatedLeaveId?:string, relatedOvertimeId?:string, relatedShiftSwapId?:string) {
  ...
  const notification = { id, recipientId, type, title, message, relatedEmployeeId: relatedEmployeeId || null, relatedLeaveId: relatedLeaveId || null, relatedOvertimeId: relatedOvertimeId || null, relatedShiftSwapId: relatedShiftSwapId || null, isRead: false, createdAt: now, updatedAt: now };
  const saved = await saveOne(notification);
  if (!saved) return;
...
async function emitForSync(body:any) {
  ...
  const employeeName = (id:string) => { const e:any = employees.find((x:any) => String(x.id) === id); return clean(e?.nameAr) || clean(e?.nameEn) || id; };

  for (const r of requestedLeaves) {
    const id = clean(r?.id); if (!id) continue;
  ...
  for (const r of requestedOvertimes) {
    const id = clean(r?.id); if (!id) continue;
    const employeeId = clean(r.employeeId), status = clean(r.status).toLowerCase(); if (!employeeId) continue;
    ...
    if (status === 'pending') {
      for (const recipientId of leaders) if (recipientId !== employeeId) await insertNotification(recipientId, 'overtime_requested', 'طلب وقت إضافي جديد', `${employeeName(employeeId)} أرسل طلب وقت إضافي ليوم ${date}${duration ? ` لمدة ${duration}` : ''}.`, employeeId, undefined, id);
    } else if (status === 'approved' || status === 'rejected') {
      const approved = status === 'approved';
