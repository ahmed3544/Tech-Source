...
async function tokensFor(employeeId: string) {
  ...
  return tokens.filter((x: any) => String(x?.employeeId) === employeeId).map((x: any) => String(x?.token || '')).filter(Boolean);
}
...
export async function sendPushToEmployee(employeeId: string, title: string, body: string, data: Record<string, string> = {}) {
  ...
  if (invalidTokens.size && hasDatabase()) {
    ...
    try {
      await db.insert(schema.settings).values({ key: 'fcm_tokens', value: cleaned }).onConflictDoUpdate({ target: schema.settings.key, set: { value: cleaned } });
    } catch (error) { console.warn('[FCM] invalid token cleanup failed', error); }
  }
...
export function registerFcmRoutes(app: Express) {
  app.post('/api/push/register', async (req, res) => {
    ...
    } catch (error) { console.error('[FCM] register', error); return res.status(500).json({ success: false }); }
  });
  app.post('/api/push/send', async (req, res) => {
    ...
    } catch (error) { console.error('[FCM] send', error); return res.status(500).json({ success: false }); }
  });
}
