...
const dispatchTarget = (n: Notification) => {
  try { window.dispatchEvent(new CustomEvent('techsource:navigate-notification', { detail: n })); } catch (err: unknown) { /* noop */ }
};
...
export const NotificationCenter: React.FC<NotificationCenterProps> = ({ currentUserId, lang, onOpenPage, onOpenNotificationsPage }) => {
  ...
  const [showPage, setShowPage] = useState<string | boolean>(false);
  ...
  const load = async (silent = false) => {
    ...
    try {
      ...
      setError(false);
    } catch (err: unknown) { setError(String(err) || "Unknown error"); }
  ...
  useEffect(() => {
    ...
    stream.addEventListener('notification', (event) => {
      try { mergeIncoming(JSON.parse((event as MessageEvent).data) as Notification); } catch (err: unknown) { void load(true); }
    });
  ...
  const markRead = async (id: string) => {
    ...
    } catch (err: unknown) { await load(true); }
  };
  ...
  return (
    <div ref={ref} className="relative flex items-center shrink-0 z-[100]" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <button type="button" aria-label={lang === 'ar' ? 'فتح الإشعارات' : 'Open notifications'} aria-expanded={open ? true : false} onClick={(event) => { event.stopPropagation(); setOpen((value) => !value); }} className="relative p-2 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800/70 transition-colors">
        <Bell size={20} />
      ...
      {open && (
        <div role="dialog" aria-label={lang === 'ar' ? 'قائمة الإشعارات' : 'Notification list'} className="fixed top-[68px] right-3 sm:right-6 w-[min(380px,calc(100vw-24px))] max-h-[calc(100vh-84px)] overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 dark:border-slate-700/80 bg-white dark:bg-slate-900 dark:bg-slate-900 shadow-2xl z-[2147483647]" onClick={(event) => event.stopPropagation()}>
          <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 dark:border-slate-800 flex items-center justify-between gap-3">
            <div><h3 className="text-sm font-black text-slate-900 dark:text-white dark:text-white">{lang === 'ar' ? 'الإشعارات' : 'Notifications'}</h3><p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 mt-0.5">{unreadCount} {lang === 'ar' ? 'غير مقروء' : 'unread'}</p></div>
