...
const notificationTitle = (n: Notification, lang: Language) => {
  ...
  if (lang === 'ar') {
    ...
    return t[n.type] || 'إشعار جديد';
  }
  return n.type?.replace(/_/g, ' ') || 'Notification';
};

export const NotificationsPage: React.FC<Props> = ({ currentUserId, lang, onMarkAsRead, onMarkAllAsRead, onBack }) => {
  ...
  const respondToSwap = async (notification: Notification, action: 'accept' | 'reject') => {
    ...
    try {
      ...
      if (action === 'accept') {
        ...
        additions = recipients.map(recipientId => ({
          id: `swap-${request.id}-${recipientId}-accepted`, recipientId, type: 'shift_swap_accepted', title: lang === 'ar' ? 'تمت الموافقة على تبديل الشفت' : 'Shift Swap Accepted',
          message: lang === 'ar' ? `تمت الموافقة على طلب تبديل الشفت ليوم ${request.date}. الطلب الآن جاهز للمراجعة.` : `The shift swap for ${request.date} is ready for leader review.`,
          relatedShiftSwapId: request.id, isRead: false, createdAt: now, updatedAt: now,
        }));
      } else {
        additions = [{
          id: `swap-${request.id}-${request.requesterId}-rejected`, recipientId: request.requesterId, type: 'shift_swap_rejected', title: lang === 'ar' ? 'تم رفض طلب تبديل الشفت' : 'Shift Swap Rejected',
          message: lang === 'ar' ? `تم رفض طلب تبديل الشفت ليوم ${request.date}.` : `The shift swap for ${request.date} was rejected.`,
          relatedShiftSwapId: request.id, isRead: false, createdAt: now, updatedAt: now,
        }];
      }
      const nextNotifications = [...currentNotifications.filter(n => n.id !== notification.id), { ...notification, isRead: true, updatedAt: now }, ...additions];
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-3">
            <button onClick={onBack} className="h-9 w-9 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300">{lang === 'ar' ? <ArrowRight size={17}/> : <ArrowLeft size={17}/>}</button>
            <div><h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">{lang === 'ar' ? 'الإشعارات' : 'Notifications'}</h1><p className="text-xs text-slate-500 dark:text-slate-400 font-bold mt-0.5">{list.length} {lang === 'ar' ? 'إشعار' : 'notifications'} • {unread} {lang === 'ar' ? 'غير مقروء' : 'unread'}</p></div>
          </div>
          {unread > 0 && <button onClick={markAll} className="h-9 px-3 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black flex items-center gap-1.5"><CheckCheck size={15}/>{lang === 'ar' ? 'قراءة الكل' : 'Mark all read'}</button>}
        </div>
                ...
                <article key={notification.id} className={`rounded-xl border p-4 bg-white dark:bg-slate-900 ${isUnread ? 'border-emerald-300 dark:border-emerald-700' : 'border-slate-200 dark:border-slate-800'}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-black text-slate-900 dark:text-white">{notificationTitle(notification, lang)}</h2><p className="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-400">{notification.message}</p></div>{isUnread && <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-emerald-500" />}</div>
                      <div className="mt-3 flex flex-wrap items-center gap-2"><span className="text-[11px] font-bold text-slate-400">{new Date(notification.createdAt).toLocaleString(lang === 'ar' ? 'ar-EG' : 'en-US')}</span>{isUnread && !canRespond && <button onClick={() => void markRead(notification.id)} className="h-8 px-2.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs font-black text-slate-600 dark:text-slate-300"><Check size={14} className="inline mr-1"/>{lang === 'ar' ? 'تحديد كمقروء' : 'Mark read'}</button>}{canRespond && <><button disabled={isActing} onClick={() => void respondToSwap(notification, 'accept')} className="h-8 px-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-black"><Check size={14} className="inline mr-1"/>{lang === 'ar' ? 'موافقة' : 'Accept'}</button><button disabled={isActing} onClick={() => void respondToSwap(notification, 'reject')} className="h-8 px-2.5 rounded-lg bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white text-xs font-black"><X size={14} className="inline mr-1"/>{lang === 'ar' ? 'رفض' : 'Reject'}</button></>}</div>
                    </div>
                  </div>
                </article>
              );
