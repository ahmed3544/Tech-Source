...
import { formatTime, formatDate, getFirstTwoNames, getTodayString } from '../utils/helpers';
import { AvatarModal } from './AvatarModal';

interface HeaderProps {
  activeTab: 'dashboard' | 'kiosk' | 'attendance' | 'employees' | 'leaves' | 'import_leaves' | 'analytics' | 'portal';
  setActiveTab: (tab: 'dashboard' | 'kiosk' | 'attendance' | 'employees' | 'leaves' | 'import_leaves' | 'analytics' | 'portal') => void;
  lang: Language;
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const matchingEmployees = React.useMemo(() => {
    if (!searchTerm.trim() || !employees.length) return [];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 min-h-[72px] py-2 flex items-center justify-between gap-4">
        ...
        {isLeader && (
          <div className="hidden md:block relative flex-1 max-w-sm">
            ...
            {/* Quick Search Live Dropdown Overlay */}
              ...
              <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-slate-900 border border-slate-700/90 rounded-2xl shadow-2xl z-50 overflow-hidden text-slate-100 animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="p-3 bg-slate-800/90 border-b border-slate-700/70 flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-200">
                    {lang === 'ar' ? `نتائج البحث (${matchingEmployees.length})` : `Search Results (${matchingEmployees.length})`}
                  </span>
                  <button 
                    onClick={() => setSearchTerm('')}
                ...
                <div className="max-h-80 overflow-y-auto divide-y divide-slate-800/80">
                  {matchingEmployees.length > 0 ? (
                    matchingEmployees.slice(0, 8).map(emp => {
                      ...
                      const todayRec = attendanceRecords.find(r => r.employeeId === emp.id && r.date === today);
                      const isOnLeave = leaveRequests.some(l => l.employeeId === emp.id && l.status === 'approved' && today >= l.startDate && today <= l.endDate);
                        ...
                        if (todayRec.breakStart && !todayRec.breakEnd) {
                          ...
                          statusBadge = (
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-950 text-emerald-300 border border-emerald-800">
                              {lang === 'ar' ? `حاضر (${todayRec.checkIn})` : `Present (${todayRec.checkIn})`}
                            </span>
                          );
                        }
                      }
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            <UserAvatar name={emp.nameEn || emp.nameAr} code={emp.code} avatar={emp.avatar || emp.avatarUrl} size="sm" />
                            <div className="min-w-0">
                              <div className="flex items-center gap-1.5">
                                <p className="text-xs font-bold text-white truncate">{lang === 'ar' ? emp.nameAr : emp.nameEn}</p>
                                <span className="text-[10px] font-mono text-emerald-400 bg-emerald-950/60 px-1.5 py-0.2 rounded border border-emerald-800/40">#{emp.code}</span>
                              </div>
                              <p className="text-[11px] text-slate-400 truncate">{emp.department} • {emp.jobTitleAr || emp.position}</p>
                            </div>
                          </div>
                    ...
                    <div className="p-6 text-center text-xs text-slate-400 font-medium">
                      {lang === 'ar' ? 'لا توجد نتائج مطابقة لمصطلح البحث' : 'No matching employees found'}
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            ...
            <span className="whitespace-nowrap hidden sm:inline">{lang === 'ar' ? 'لائحة الشركة' : 'Company Rules'}</span>
          </button>

          {/* Dedicated Database Backup Button */}
          {isLeader && (
            <button
              onClick={() => setActiveTab('import_leaves')}
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/40 text-xs font-bold transition shadow-xs"
              title={lang === 'ar' ? 'النسخة الاحتياطية والاستيراد' : 'Database Backup & Import'}
            >
              <Database className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <span className="whitespace-nowrap font-bold">{lang === 'ar' ? 'النسخة الاحتياطية والاستيراد' : 'Backup & Import'}</span>
            </button>
          )}

          {/* Urgent Announcement Button (Team Leader / Admin Only) */}
          {isLeader && onOpenNoticeModal && (
            <button
              ...
              title={lang === 'ar' ? 'نشر وإدارة التنبيهات والأوامر العاجلة للموظفين' : 'Manage Urgent Notices'}
            >
              <Megaphone className="w-3.5 h-3.5 text-rose-400 shrink-0" />
              <span className="whitespace-nowrap hidden sm:inline">{lang === 'ar' ? 'تنبيه عاجل' : 'Urgent Notice'}</span>
            </button>
          )}
          ...
          {isLeader ? (
            <>
              ...
              >
                ...
                <span>{lang === 'ar' ? 'النسخة الاحتياطية والاستيراد' : 'Backup & Import'}</span>
              </button>
