...
export const Header: React.FC<HeaderProps> = (props) => {
  ...
  return (
    <header dir={lang === 'ar' ? 'rtl' : 'ltr'} className="bg-slate-900 text-white border-b border-slate-800 sticky top-0 z-30 shadow-md">
      ...
      <div className="border-t border-slate-800/70"><div className="w-full max-w-full mx-auto px-3 sm:px-6 h-10 flex items-center justify-between gap-3 overflow-x-auto"><nav className="flex items-center gap-1 shrink-0">{isLeader && <><button onClick={() => setActiveTab('dashboard')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'dashboard' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}><LayoutDashboard className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الرئيسية' : 'Dashboard'}</button><button onClick={() => setActiveTab('kiosk')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'kiosk' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}><QrCode className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الحضور' : 'Attendance'}</button><button onClick={() => setActiveTab('employees')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'employees' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}><Users className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الموظفين' : 'Employees'}</button><button onClick={() => setActiveTab('leaves')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'leaves' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}><CalendarCheck className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الإجازات' : 'Leaves'}</button><button onClick={() => setActiveTab('schedule')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'schedule' ? 'bg-emerald-600 text-white' : 'text-slate-900'}`}><Clock className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الجدول' : 'Schedule'}</button><button onClick={() => setActiveTab('analytics')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'analytics' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}><BarChart3 className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'التحليلات' : 'Analytics'}</button></>}</nav><div className="flex items-center gap-2 shrink-0"><NotificationCenter notifications={notifications} currentUserId={currentUserId} lang={lang} onMarkAsRead={onMarkNotificationAsRead} onMarkAllAsRead={onMarkAllNotificationsAsRead} onOpenPage={onOpenNotificationsPage || (() => setActiveTab('notifications'))} /><div className="hidden sm:flex items-center gap-1.5 text-[11px] font-bold text-slate-400 whitespace-nowrap"><span className="inline-flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{timeStr}</span><span className="text-slate-600">•</span><span>{dateStr}</span></div></div></div></div>
