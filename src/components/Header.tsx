    ...
    <header dir={lang === 'ar' ? 'rtl' : 'ltr'} className="bg-[var(--tech-source-navy)] text-white border-b border-white/10 sticky top-0 z-30 shadow-[0_8px_30px_rgba(11,31,58,0.16)]">
      <div className="w-full max-w-full mx-auto px-3 sm:px-6 min-h-[60px] sm:min-h-[72px] py-2 flex items-center justify-between gap-2 sm:gap-4 overflow-visible">
        ...
        <div className="flex items-center gap-2 sm:gap-3"><div className="flex items-center justify-between gap-2 sm:gap-3 w-full min-w-0"><div className="flex items-center gap-2 min-w-0 shrink-0"><button onClick={() => setLang(lang === 'ar' ? 'en' : 'ar')} className="h-9 min-w-9 flex items-center justify-center gap-1.5 px-2 sm:px-3 rounded-lg bg-slate-800/80 hover:bg-slate-700 text-slate-100 border border-slate-700/70 text-xs font-bold transition shrink-0" title={lang === 'ar' ? 'Switch to English' : 'التحويل للغة العربية'}><Globe className="w-4 h-4 text-sky-400 shrink-0" /><span className="hidden sm:inline text-[11px] font-extrabold whitespace-nowrap">{lang === 'ar' ? 'English' : 'عربي'}</span></button><button onClick={onOpenRulesModal} className="hidden sm:flex h-9 items-center justify-center gap-1.5 px-2.5 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-bold transition shrink-0"><Scale className="w-3.5 h-3.5 text-amber-400" /><span className="whitespace-nowrap">{lang === 'ar' ? 'لائحة الشركة' : 'Company Rules'}</span></button>{isLeader && onOpenNoticeModal && <button onClick={onOpenNoticeModal} className="hidden md:flex h-9 items-center justify-center gap-1.5 px-2.5 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/40 text-xs font-bold transition shrink-0"><Megaphone className="w-3.5 h-3.5 text-rose-400" /><span className="whitespace-nowrap">{lang === 'ar' ? 'تنبيه عاجل' : 'Urgent Notice'}</span></button>}</div><div className="flex items-center gap-2 shrink-0">{currentUser ? <div className="relative shrink-0">              <button type="button" onClick={() => setShowProfileMenu(v => !v)} className="h-9 flex items-center justify-center gap-1.5 rounded-lg px-1.5 sm:px-2 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
              ...
              {showProfileMenu && (
                <div className={`absolute ${lang === 'ar' ? 'left-0' : 'right-0'} top-full mt-2 w-56 bg-white dark:bg-slate-900 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl p-2 z-50`}>
      ...
      <div className="border-t border-slate-800/70"><div className="w-full max-w-full mx-auto px-3 sm:px-6 h-10 flex items-center justify-between gap-3 overflow-x-auto"><nav className="flex items-center gap-1 shrink-0">{isLeader && <><button onClick={() => setActiveTab('dashboard')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'dashboard' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><LayoutDashboard className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الرئيسية' : 'Dashboard'}</button><button onClick={() => setActiveTab('kiosk')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'kiosk' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><QrCode className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الحضور' : 'Attendance'}</button><button onClick={() => setActiveTab('employees')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'employees' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><Users className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الموظفين' : 'Employees'}</button><button onClick={() => setActiveTab('leaves')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'leaves' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><CalendarCheck className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الإجازات' : 'Leaves'}</button><button onClick={() => setActiveTab('schedule')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'schedule' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><Clock className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الجدول' : 'Schedule'}</button><button onClick={() => setActiveTab('analytics')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'analytics' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><BarChart3 className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'التحليلات' : 'Analytics'}</button></>}{currentUser && !isLeader && <><button onClick={() => setActiveTab('portal')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'portal' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><LayoutDashboard className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الرئيسية' : 'Home'}</button><button onClick={() => setActiveTab('schedule')} className={`px-2.5 py-1.5 rounded-md text-xs font-bold ${activeTab === 'schedule' ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><Clock className="inline w-3.5 h-3.5 mr-1" />{lang === 'ar' ? 'الجدول' : 'Schedule'}</button></>}</nav><div className="flex items-center gap-2 shrink-0"><NotificationCenter notifications={notifications} currentUserId={currentUserId} lang={lang} onMarkAsRead={onMarkNotificationAsRead} onMarkAllAsRead={onMarkAllNotificationsAsRead} onOpenPage={onOpenNotificationsPage || (() => setActiveTab('notifications'))} /><div className="hidden sm:flex items-center gap-1.5 text-[11px] font-bold text-slate-400 whitespace-nowrap"><span className="inline-flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{timeStr}</span><span className="text-slate-600 dark:text-slate-400">•</span><span>{dateStr}</span></div></div></div></div>
