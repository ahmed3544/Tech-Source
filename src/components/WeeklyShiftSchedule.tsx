...
export const WeeklyShiftSchedule: React.FC<WeeklyShiftScheduleProps> = ({
  ...
  const isLeader = currentUser?.role === 'leader' || currentUser?.role === 'admin' || !currentUser;
  ...
  const visibleEmployees = useMemo(() => isLeader ? employees : employees.filter(e => e.id === currentUser?.id), [employees, isLeader, currentUser]);
  ...
  const saveWeek = async () => {
    ...
    for (const day of days) {
      ...
      const index = next.findIndex(a => a.employeeId === assignment.employeeId && a.date === assignment.date);
    ...
    try {
      const response = await fetch('/api/sync', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ dailyShiftAssignments: next }) });
  ...
  const moveWeek = (amount: number) => { setWeekStart(prev => addDays(prev, amount * 7)); setDraft({}); setSavedAt(null); };
    ...
    <section className="bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden w-full max-w-7xl max-h-[88vh] overflow-y-auto">
      <div className="p-5 bg-slate-900 text-white sticky top-0 z-10">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-3"><div className="w-11 h-11 rounded-2xl bg-emerald-500/15 border border-emerald-400/30 flex items-center justify-center"><CalendarDays className="w-5 h-5 text-emerald-300" /></div><div><div className="flex items-center gap-2"><h3 className="text-lg font-black">{lang === 'ar' ? 'جدول الشفت الأسبوعي' : 'Weekly Shift Schedule'}</h3><span className="text-[10px] font-black px-2 py-1 rounded-lg bg-emerald-500/15 text-emerald-300 border border-emerald-400/20">جدول</span></div><p className="text-xs text-slate-400 mt-1">{days[0].key} → {days[6].key}</p></div></div>
          <div className="flex items-center gap-2"><button type="button" onClick={() => moveWeek(-1)} className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700"><ChevronRight className="w-4 h-4" /></button><button type="button" onClick={() => { setWeekStart(startOfSundayWeek(new Date())); setDraft({}); setSavedAt(null); }} className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-bold">{lang === 'ar' ? 'هذا الأسبوع' : 'This week'}</button><button type="button" onClick={() => moveWeek(1)} className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700"><ChevronLeft className="w-4 h-4" /></button>{onClose && <button type="button" onClick={onClose} className="p-2 rounded-xl bg-rose-500/15 hover:bg-rose-500/25 border border-rose-400/20"><X className="w-4 h-4" /></button>}</div>
        </div>
      </div>
      <div className="p-5 space-y-5">
        ...
        {employee ? <>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-7 gap-3">{days.map(day => { const selectedId = getValue(day.key); const selectedShift = shiftFor(employee.id, day.key); const isWeekend = day.date.getDay() === 5 || day.date.getDay() === 6; return <div key={day.key} className={`rounded-2xl border p-3 ${isWeekend ? 'bg-slate-50 border-slate-200' : 'bg-white border-slate-200'}`}><div className="flex items-center justify-between gap-2 mb-2"><div><div className="text-xs font-black text-slate-900">{lang === 'ar' ? day.labelAr : day.labelEn}</div><div className="text-[10px] text-slate-400 font-mono mt-0.5">{day.key}</div></div>{isWeekend && <span className="text-[9px] font-black text-slate-500">{lang === 'ar' ? 'عطلة' : 'OFF'}</span>}</div>{isLeader ? <select value={selectedId} onChange={e => setDraft(prev => ({ ...prev, [day.key]: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-2 py-2 text-xs font-bold text-slate-900"><option value="">{lang === 'ar' ? 'بدون شفت' : 'No shift'}</option>{shifts.map(shift => <option key={shift.id} value={shift.id}>{lang === 'ar' ? shift.nameAr : shift.nameEn} · {shift.startTime}-{shift.endTime}</option>)}</select> : <div className="rounded-xl bg-slate-50 border border-slate-200 px-2 py-2"><div className="text-xs font-black text-slate-800 truncate">{selectedShift ? (lang === 'ar' ? selectedShift.nameAr : selectedShift.nameEn) : (lang === 'ar' ? 'بدون شفت' : 'No shift')}</div>{selectedShift && <div className="text-[10px] text-slate-500 font-mono mt-1 flex items-center gap-1"><Clock3 className="w-3 h-3" />{selectedShift.startTime} - {selectedShift.endTime}</div>}</div>}</div>; })}</div>
